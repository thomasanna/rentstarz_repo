...
class Video_IndexController extends Core_Controller_Action_Standard
{
  ...
  public function createAction()
  {
	...
	public function jqueryuploadVideoAction()
	{
		...
		$this->view->feeduploads=$feeduploads=$this->_getParam('feeduploads');
		$upload             = new Zend_File_Transfer_Adapter_Http();
		$filename           = $upload->getFileName(null,false);
		$filemtype          = $upload->getMimeType();
		$filesize           = $upload->getFileSize();
		$originalFilename   = pathinfo($filename);
		$extensionUploaded  = strtolower($originalFilename['extension']);

		$uniquefilename                 =   uniqid() ;
		$newFilename                    =  $uniquefilename. '.' . $originalFilename['extension'];
		$target                         ='public/video/uploaded/'.$newFilename;
		//$originalFilename['extension']  = 'mp4';
		$newTargetFilename              =  $uniquefilename. '.' . $originalFilename['extension'];
		$newtarget                      ='public/video/uploaded/'.$uniquefilename.'.'.$originalFilename['extension'];

		$newFilename =  uniqid() . '.' . $originalFilename['extension'];

		$VideoDirectory = "public/video/uploaded/";
		if (!file_exists($VideoDirectory)) {
			mkdir($VideoDirectory, 0777);
		}
		...
		$upload->receive();
		#$convert_command  = 'ffmpeg -i '.$target.' -ar 22050 -ab 32 -f mp4 - | flvtool2 -U stdin '.$newtarget.'';
		#$convert_command  = 'ffmpeg -i '.$target.' -b:a 128k -vcodec mpeg4 -b:v 1200k '.$newtarget.'';
		if($extensionUploaded == "flv")
		//$convert_command  = 'ffmpeg -i '.$target.' -ar 22050 '.$newtarget.'';
		$newtarget = $target;
		else if($extensionUploaded == "avi")
		$convert_command  = 'ffmpeg -i '.$target.' -b 300 -s 320x240 -vcodec xvid -ab 32 -ar 24000 -acodec aac '.$newtarget.' ';
		else if($extensionUploaded == "mp4")
		$newtarget = $target;
		else
		$convert_command  = 'ffmpeg -i '.$target.' -b:a 128k -vcodec mpeg4 -b:v 1200k '.$newtarget.'';
		...
		$filemtype  =   mime_content_type($newtarget);
		$filesize   =   filesize($newtarget);

		if($filemtype == "")
		$filemtype = "inode/x-empty";

		if($filesize == "")
		$filesize   = 0;

		$files= array(
									  'url'=> $this->get_full_url().'/'.$target,
									  'name'=> $newTargetFilename,
									  'type'=> $filemtype,
		...
		try
		{
		  ...
		  $video = Engine_Api::_()->video()->jquerycreateVideo($params, $files, $values);
		  ...
		  $db->commit();
		   $_SESSION['video_'.$shareupload]= $video->video_id;
		  $res['status'] = true;
		  $res['video_id'] = $video->video_id;
		  $res['photo_id'] = $video->photo_id;
		  $res['title'] = $video->title;
		  $res['description'] = $video->description;
		  $res['src'] = $video->getPhotoUrl();
		  $res['message'] = Zend_Registry::get('Zend_Translate')->_('Video posted successfully');
	...
	public function uploadpreviewAction(){
		...
		$this->_helper->layout->disableLayout();
		$this->view->shareupload=$shareupload=$this->_getParam('shareuploads');
		$video_id = $_SESSION['video_'.$shareupload];
		...
		$photo = Engine_Db_Table::getDefaultAdapter()->select()
						  ...
						  ->where('type = ?','thumb')
						  ->query()->fetch();
				/*$photo   =   $storageTable->select()
				 ->from('engine4_storage_files')
						  ->where('parent_id = ?',$videoid)
						  ->where('type = ?','thumb');                                             

    	$propertyVideoData=$storageTable->fetchRow($photo);  */
		//$this->view->src = $photo['storage_path'];
		//$resultArray	=	array($photo);
		array_push($resultArray, $photo);
	}
		/*$videoSelect = Engine_Db_Table::getDefaultAdapter()->select()
						  ->from('engine4_storage_files')
						  ->where('parent_id = ?',$videoid)
		...
		$this->view->videoResult	=	$videoSelect;*/
		
		}  
	
$this->view->videoResult	=	$resultArray;
/*print_r($this->view->videoResult);exit;*/
		/*foreach($videoids as $videoid)
		{
		//if($video->video_id){
		$videoSelect = Engine_Db_Table::getDefaultAdapter()->select()
		...
		$this->view->videoResult	=	$videoSelect;
		/*$this->view->type = $videoSelect['parent_type'];
		$this->view->file_id = $videoSelect['file_id'];
		$this->view->video_id = $videoid;
		}                  
		//$res['status'] = true;
