...
<script type="text/javascript">
  en4.core.runonce.add(function() {
	...
	up = new FancyUpload2($('demo-status'), $('demo-list'), { // options object
      ...
      url: jQuery('form-upload').action + '?ul=1',
      ...
      onLoad: function() {
        jQuery('#demo-status').removeClass('hide'); // we show the actual UI
        //jQuery('#demo-fallback').destroy(); // ... and hide the plain form
      ...
      onSelectSuccess: function(file) {
        //jQuery('demo-list').style.display = 'block';
        jQuery('#demo-list').css('display','block');
        var demoupload = document.getElementById("demo-upload");
        var demobrowse = document.getElementById("demo-browse");
        ...
        jQuery('#demo-status-overall').css('display','block');
        //up.start();
      } ,
      /**
      ...
      onFileSuccess: function(file, response) {
        ...
        if (json.get('status') == '1') {
          ...
          jQuery('code').value=json.get('code');
          jQuery('id').value=json.get('video_id');
    ...
    var startUpload = window.startUpload = function() {
      jQuery('type-wrapper').style.display = "none";
      jQuery('demo-upload').style.display = "none";
      up.start();
    }
  });

  function showSaveButton(){
     if(!jQuery('#input1').value){
       jQuery('#upload-wrapper').hide();
     }
     else {
       jQuery('#upload-wrapper').show();
     }
  }
</script>
