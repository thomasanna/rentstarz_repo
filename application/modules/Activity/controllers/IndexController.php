...
class Activity_IndexController extends Core_Controller_Action_Standard
{
  ...
  function deleteAction()
  {
        ...
        ('user' == $action->object_type  && $viewer->getIdentity() == $action->object_id)))   // commenter
    {
      ...
      $db->beginTransaction();
      try {
		   if( $action->getTypeInfo()->type=="property_requirement")
		   {
			   
			   Engine_Db_Table::getDefaultAdapter()->delete('engine4_property_requirement', array('action_id = ?'=>$action_id) );
		   } 
		   if( $action->getTypeInfo()->type=="property")
		   {
			$propertyTable =  Engine_Api::_()->getDbtable('propertylist', 'user');	  
			$propertyData = $propertyTable->fetchRow($propertyTable->select()->where('action_id = ?', $action_id));
			
	     
			 Engine_Db_Table::getDefaultAdapter()->delete('engine4_property_images', array('property_id = ?'=>$propertyData->id) );
			 Engine_Db_Table::getDefaultAdapter()->delete('engine4_property_granted', array('property_id = ?'=>$propertyData->id) );
			 Engine_Db_Table::getDefaultAdapter()->delete('engine4_property_request', array('property_id = ?'=>$propertyData->id) );
			 Engine_Db_Table::getDefaultAdapter()->delete('engine4_property_list', array('id = ?'=>$propertyData->id) );
        ...
        // tell smoothbox to close
        $this->view->status  = true;
        $this->view->message = Zend_Registry::get('Zend_Translate')->_('This post has been removed.');
        $this->view->smoothboxClose = true;
        return $this->render('deletedItem');
      } catch (Exception $e) {
        $db->rollback();
    ...
    $this->view->form = $form = new Activity_Form_Editpost(array(
    ...
    $body = $form->getValue('editpostbody');
    $body = preg_replace('/<br[^<>]*>/', "\n", $body); 
