...
class Activity_Model_DbTable_Actions extends Engine_Db_Table
{
  ...
  {    
    ...
    foreach( $masterActionTypes as $type ) {
      if( $type->displayable & 4 ) {
		...
		if($profile_type_id == 4){ // landlords
			 
			if($type->type == 'property_requirement'){
		...
		elseif($profile_type_id == 1){ // tenants
			...
			if($type->type == 'property' || $type->type == 'property_requirement'){
          ...
          array $params = array())
  {
    ...
    $objectActionTypes = array();
     ...
     $fieldsByAlias = Engine_Api::_()->fields()->getFieldsObjectsByAlias($about); 
    	
    	if( !empty($fieldsByAlias['profile_type']) )
			{
			  $optionId = $fieldsByAlias['profile_type']->getValue($about);
			  $profile_type_id = $optionId->value; 
			}
	...
	/*	if($type->type != 'friends' && $type->type != 'signup'){  
        $subjectActionTypes[] = $type->type;
	   }*/
      }
