...
class User_Model_User extends Core_Model_Item_Abstract
{
  ...
  public function getTitle()
  {
    ...
    if( isset($this->displayname) && '' !== trim($this->displayname) ) {
     ...
     // return $tmp[0];
      ...
      $email = $this->email;
			$name  = ucwords(strstr($email,'@', true));				
			return	$username  = ucwords(str_replace("."," ",$name));	
    } else {
  ...
  public function setDisplayName($displayName)
  {
    ...
    else if( is_array($displayName) )
    {
      ...
      $this->displayname = ucwords($displayName);
    }
  }
  ...
  public function setPhoto($photo)
  {
    ...
    // Resize image (normalsquare)
    $passportPath = $path . DIRECTORY_SEPARATOR . $base . '_m.' . $extension;
    $image = Engine_Image::factory();
    $image->open($file)
      ->autoRotate()
      ->resize(120, 126)
    ...
    $iPassport = $filesTable->createFile($passportPath, $params);
