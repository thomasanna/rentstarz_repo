...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	function hooks_message($userid,$to,$unsanitizedmessage,$dir) {
		...
		if( strpos($unsanitizedmessage,'acceptAVChat')!=false){
			$sql = ("SELECT lang_text from cometchat_languages where lang_key = 'av_chat_request'");
			$query = mysqli_query($GLOBALS['dbh'],$sql);
			$result = mysqli_fetch_assoc($query);
			$unsanitizedmessage = $result['lang_text'];
		}

		if( strpos($unsanitizedmessage,'PLUGIN_AVCHAT_CANCELCALL')!=false){
			$sql = ("SELECT lang_text from cometchat_languages where lang_key = 'call_cancelled'");
			$query = mysqli_query($GLOBALS['dbh'],$sql);
			$result = mysqli_fetch_assoc($query);
			$unsanitizedmessage = $result['lang_text'];
		}

		if( strpos($unsanitizedmessage,'has accepted your audio/video chat request')!=false){
			$sql = ("SELECT lang_text from cometchat_languages where lang_key = 'request_accepted'");
			$query = mysqli_query($GLOBALS['dbh'],$sql);
			$result = mysqli_fetch_assoc($query);
			$unsanitizedmessage = $result['lang_text'];
		}

		if( strpos($unsanitizedmessage,'PLUGIN_AVCHAT_REJECTCALL')!=false){
			$sql = ("SELECT lang_text from cometchat_languages where lang_key = 'user_busy_1'");
			$query = mysqli_query($GLOBALS['dbh'],$sql);
			$result = mysqli_fetch_assoc($query);
			$unsanitizedmessage = $result['lang_text'];
		}

		if( strpos($unsanitizedmessage,'has shared his/her screen with you')!=false){
			$sql = ("SELECT lang_text from cometchat_languages where lang_key = 'sent_success_other'");
			$query = mysqli_query($GLOBALS['dbh'],$sql);
			$result = mysqli_fetch_assoc($query);
			$unsanitizedmessage = $result['lang_text'];
		}

		if( strpos($unsanitizedmessage,'has sent you an audio chat request')!=false){
			$sql = ("SELECT lang_text from cometchat_languages where lang_key = 'audio_chat_request'");
			$query = mysqli_query($GLOBALS['dbh'],$sql);
			$result = mysqli_fetch_assoc($query);
			$unsanitizedmessage = $result['lang_text'];
		}
		...
		$decoded_cc_message = decode_controlmessage($unsanitizedmessage);
			if(!empty($decoded_cc_message)){
				if($decoded_cc_message['name'] == 'stickers'){
					$sql = ("SELECT lang_text from cometchat_languages where lang_key = 'sticker_received'");
					$query = mysqli_query($GLOBALS['dbh'],$sql);
					$result = mysqli_fetch_assoc($query);
					$unsanitizedmessage = $result['lang_text'];
				}else{
		...
		if(!empty($unsanitizedmessage)){
			$sql = ("SELECT DISTINCT c.title,c.conversation_id from ".TABLE_PREFIX."messages_conversations c join ".TABLE_PREFIX."messages_messages m on c.conversation_id = m.conversation_id where ((c.user_id = ".$userid." and m.user_id = ".$to.") or (m.user_id = ".$userid." and c.user_id = ".$to.")) and c.title = 'chat'");
			$query = mysqli_query($GLOBALS['dbh'],$sql);
			$result = mysqli_fetch_assoc($query);
