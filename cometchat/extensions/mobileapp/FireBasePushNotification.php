...
class FireBasePushNotification {
	...
	public function sendNotification($channel, $messageData, $isChatroom = '0', $isAnnouncement = '0',$isWRTC = '0') {
		if(function_exists('curl_version') && !empty($channel) && $messageData ) {
			...
			$notificationData = array_merge(array('alert' => $messageData['m'],'t' => $messageType,'m' => $messageData,'action' => "PARSE_MSG",'sound' => $soundfile,"title" => "CometChat", "badge" => "Increment" , "isFirebaseNotification" => '1'),$notificationData);
			...
			$pushPayload['notification'] = array('text' => $messageData['m'],'title'=>'CometChat','icon'=>'ic_launcher_small','sound' => $soundfile);
